...
val_loader = DataLoader(val, batch_size=args.test_batch_size, pin_memory=args.pin_memory, num_workers=args.n_workers, shuffle=False, drop_last=True)
...
for k,v in zip(script_utils.PARAMS[:-10] + ['N_TRAIN', 'N_TEST', 'TEST_BATCH_SIZE', 'TRAIN_SIZE', 'VAL_SIZE', 'TRAIN_MU', 'TRAIN_SIG'], values + [args.n_testtrain, args.n_testtest, args.test_batch_size, len(train), len(val), mu, sig]):
